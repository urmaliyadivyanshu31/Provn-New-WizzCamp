      ...
      if (mintResult && mintResult.tokenId) {
            ...
            if (metadata) {
              ...
              try {
                ...
                const syncPayload = {
                  ...
                  title,
                  description:
                    description || `A video uploaded via Provn platform`,
                ...
                console.log("🔄 Sync data being sent:", {
                  ...
                  originalPreviewSize: preview
                    ? preview.length + " chars"
                    : "none",
                  isPreviewDataUrl: preview
                ...
                if (syncData.success && syncData.synced) {
                  ...
                  if (syncData.action_required?.includes("profile")) {
                    toast.error(
                      "Please create a profile first to sync videos!"
                    );
                  } else {
                    toast.warning(
                      "Video minted but not synced to profile. Check console for details."
                    );
                  }
                } else {
                  console.error("❌ Sync failed:", syncData.error);
                  toast.error(
                    "Video minted but sync failed. Check your profile later."
                  );
                }
              } catch (syncError) {
        ...
        try {
          const videos = JSON.parse(
            localStorage.getItem("provn-videos") || "[]"
          );
          const newVideo = {
        ...
        <div className="space-y-8">
          ...
          {authenticated && (
            <div className="text-center">
              <ProvnBadge variant="success" className="text-sm px-3 py-1">
                ✓ Wallet Connected
              </ProvnBadge>
            </div>
          )}
