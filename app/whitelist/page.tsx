...
import { useAuth, useModal, useConnect, CampModal } from '@campnetwork/origin/react'
  ...
  const handleWalletAuth = async () => {
    ...
    try {
      ...
      if (result.success && result.hasAccess) {
        ...
        document.cookie = `wallet_address=${walletAddress}; path=/; max-age=86400; SameSite=Lax; Secure=${location.protocol === 'https:'}`
        ...
        setSubmissionResult({
          success: false,
          message: result.message || 'This wallet is not whitelisted. Please use email or X to request access.',
          type: 'wallet'
        })
      }
    } catch (error) {
  ...
  const handleEmailSubmission = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setSubmissionResult({
          success: true,
          message: 'Request submitted successfully. We\'ll notify you when access is available.',
          type: 'email'
        })
        setEmail('')
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <div className="w-full p-6">
          <div className="text-center">
            <p className="text-xs text-provn-muted">
              Powered by{' '}
              <span className="text-provn-accent font-medium">Camp Network</span>
            </p>
          </div>
        </div>
      </div>
